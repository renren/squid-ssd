...
storeCossAllocate(SwapDir * SD, const StoreEntry * e, int which)
{
    ...
    } else if ((cs->current_offset + allocsize) > ((off_t) cs->max_size << 10)) {
	/*
...
storeCossCreateMemOnlyBuf(SwapDir * SD)
{
    ...
    newmb->diskstart = ((off_t) cs->max_size << 10) + start;
    newmb->stripe = stripe;
